<script setup>
  ...
  const activeClass = ( route_ ) => {
    ...
    // console.log( 'PE::001' , route.currentRoute.value.path )
  ...
  const activeClassIcon = ( route_ ) => {
    ...
    if ( val_ ) {
      return 'bg-gradient-to-tl from-purple-700 to-pink-500 shadow-soft-2xl mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-white bg-center stroke-0 text-center xl:p-2.5';
    } else {
  ...
  const hasRoute = ( route_ ) => {
    let route_current = route.currentRoute.value.path;
    return route_ === route_current ? 'white' : '#e20e7d' ;
  };
...
<template>
  ...
  <aside class="max-w-62.5 ease-nav-brand z-990 fixed inset-y-0 my-4 ml-4 block w-full -translate-x-full flex-wrap items-center justify-between bg-white overflow-y-auto rounded-2xl border-0 p-0 antialiased shadow-none transition-transform duration-200 xl:left-0 xl:translate-x-0 ">
    
    <div class="h-19.5">
      <i class="absolute top-0 right-0 hidden p-4 opacity-50 cursor-pointer fas fa-times text-white-400 xl:hidden" sidenav-close></i>
      <NuxtLink to="/dashboard"  class="block px-8 py-6 m-0 text-sm whitespace-nowrap text-slate-700" >
        <img src="/assets/admin/img/lilipink_yoi_logo_sm.png" class="inline h-full max-w-full transition-all duration-200 ease-nav-brand max-h-8" alt="main_logo" />
        <!-- <img src="/assets/admin/img/logo-ct.png" class="inline h-full max-w-full transition-all duration-200 ease-nav-brand max-h-8" alt="main_logo" /> -->
    ...
    <div class="items-center block w-auto max-h-screen overflow-auto h-sidenav grow basis-full" >
      <ul class="flex flex-col pl-0 mb-0">
        ...
        <li class="w-full mt-4">
          <h6 class="pl-6 ml-2 text-xs font-bold leading-tight uppercase opacity-60">Lili Pink & Yoi</h6>
        </li>

        <li class="mt-0.5 w-full">
          <NuxtLink to="clientes" :class="activeClass( '/clientes' )"  >
            <div :class="activeClassIcon( '/clientes' )">
              <i class="fas fa-users" :style="'color: '+hasRoute( '/clientes' )+' ;'"></i>
            </div>
            <span class="ml-1 duration-300 opacity-100 pointer-events-none ease-soft">Clientes</span>
          </NuxtLink>
        </li>
        <li class="mt-0.5 w-full">
          <NuxtLink to="bonos" :class="activeClass( '/bonos' )" >
            <div :class="activeClassIcon( '/bonos' )">
              <i class="far fa-credit-card" :style="'color: '+hasRoute( '/bonos' )+' ;'"></i>
            </div>
            <span class="ml-1 duration-300 opacity-100 pointer-events-none ease-soft" >Bonos</span>
          </NuxtLink>
        </li>
