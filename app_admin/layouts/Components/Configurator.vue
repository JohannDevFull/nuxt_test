<script setup>
  ...
  import { useThemeAdminStore } from '@/stores/theme_admin'

  const theme_admin = useThemeAdminStore();
  ...
  const navbarFixed = () => {
    ...
    if ( theme_admin.navbar_fixed )
    {
      // alert( 'SII::'+ theme_admin.navbar_fixed )
      ...
      navmenu.classList.add("bg-llpyoi");
  ...
  const changeFixed = () => {
    theme_admin.navbar_fixed = !theme_admin.navbar_fixed;
...
<template>
  ...
  <div fixed-plugin>
    ...
    <div fixed-plugin-card class="z-sticky shadow-soft-3xl w-90 ease-soft right-0 fixed top-0 left-auto flex h-full min-w-0 flex-col break-words rounded-none border-0 bg-white bg-clip-border px-2.5 duration-200">
      ...
      <div class="flex-auto p-6 pt-0 sm:pt-4">
        ...
        <!-- Navbar Fixed -->
        <div class="mt-4">
          <h6 class="mb-0">Navbar Fixed: 
            (  {{ theme_admin.navbar_fixed }}  )
          </h6>
        </div>
        <div class="min-h-6 mb-0.5 block pl-0">
          <input navbarFixed @change="changeFixed()" class="rounded-10 duration-250 ease-soft-in-out after:rounded-circle after:shadow-soft-2xl after:duration-250 checked:after:translate-x-5.25 h-5 relative float-left mt-1 ml-auto w-10 cursor-pointer appearance-none border border-solid border-gray-200 bg-slate-800/10 bg-none bg-contain bg-left bg-no-repeat align-top transition-all after:absolute after:top-px after:h-4 after:w-4 after:translate-x-px after:bg-white after:content-[''] checked:border-slate-800/95 checked:bg-slate-800/95 checked:bg-none checked:bg-right" type="checkbox" :checked="theme_admin.navbar_fixed" />
        </div>
